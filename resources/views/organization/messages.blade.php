    ...
    <div class="welcome-header">
        <div class="welcome-avatar">
            @if(auth()->user()->organization && auth()->user()->organization->logo_image)
                <img src="{{ asset('storage/' . auth()->user()->organization->logo_image) }}" alt="Avatar" style="width: 100%; height: 100%; object-fit: cover; border-radius: 50%;">
            @else
        ...
        <div class="welcome-text">
            <h1>Welcome back, {{ auth()->user()->organization->org_name ?? 'Organization' }}!</h1>
            <p>Manage your conversations with volunteers.</p>
        </div>
    </div>
</div>
        ...
        <div style="width: 35%; border-right: 2px solid #E5E7EB; overflow-y: auto;">
            ...
            <div>
                ...
                @else
                    ...
                    <a href="{{ route('organization.messages', ['user_id' => $conversation['user']->user_id]) }}" 
                       ...
                       onmouseout="this.style.background='{{ request('user_id') == $conversation['user']->user_id ? '#F9FAFB' : 'transparent' }}'">
                        <div style="display: flex; align-items: start; gap: 12px;">
                            <!-- Avatar -->
                            <div style="width: 45px; height: 45px; border-radius: 50%; background: #A7E8D4; display: flex; align-items: center; justify-content: center; flex-shrink: 0; color: #2C3E50; font-weight: 600; font-size: 14px;">
                                {{ strtoupper(substr($conversation['user']->volunteer->first_name ?? $conversation['user']->email, 0, 2)) }}
                            </div>
                            ...
                            <div style="flex: 1; min-width: 0;">
                                <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 4px;">
                                    <h3 style="font-size: 14px; font-weight: 600; color: #2C3E50; margin: 0; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">
                                        {{ $conversation['user']->volunteer->first_name ?? 'Volunteer' }} {{ $conversation['user']->volunteer->last_name ?? '' }}
                                    </h3>
            ...
            @if($selectedUser)
                ...
                <div style="padding: 20px 25px; border-bottom: 2px solid #E5E7EB; background: #F9FAFB;">
                    <div style="display: flex; align-items: center; gap: 12px;">
                        <div style="width: 45px; height: 45px; border-radius: 50%; background: #A7E8D4; display: flex; align-items: center; justify-content: center; color: #2C3E50; font-weight: 600; font-size: 14px;">
                            {{ strtoupper(substr($selectedUser->volunteer->first_name ?? $selectedUser->email, 0, 2)) }}
                        </div>
                        <div>
                            <h3 style="font-size: 16px; font-weight: 600; color: #2C3E50; margin: 0;">
                                {{ $selectedUser->volunteer->first_name ?? 'Volunteer' }} {{ $selectedUser->volunteer->last_name ?? '' }}
                            </h3>
                            <p style="font-size: 12px; color: #6B7280; margin: 0;">Volunteer</p>
                        </div>
                    </div>
                </div>
                ...
                <div style="padding: 20px 25px; border-top: 2px solid #E5E7EB; background: white;">
                    <form method="POST" action="{{ route('organization.messages.send') }}" style="display: flex; gap: 12px; align-items: center;">
                        @csrf
